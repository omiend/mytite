...
object Application extends Controller with Secured {
  ...
  def index(pageNum: Int) = DBAction { implicit request =>
    ...
    // データリスト
    pager.dataList = TwitterUser.findByOffset(pager.pageNum * pager.maxListCount - pager.maxListCount, pager.maxListCount)
    // 全体件数
    pager.totalRows = TwitterUser.count
   ...
   * Festival一覧画面起動
   *****************************************************************************/
  def festival(pageNum: Int, twitterScreenName: String) = DBAction { implicit request =>
    // CookieからTwitterIdを取得し、取得出来た場合TwitterUserを取得する
    var twitterUser: Option[TwitterUser] = request.session.get("twitterId") match {
      case Some(twitterId) => TwitterUser.findByTwitterId(twitterId.toLong)
      case _ => None
    ...
    TwitterUser.findByTwitterScreenName(twitterScreenName) match {
      case Some(targetTwitterUser) => {
        ...
        val resultTuple = 
        // データリスト
        pager.dataList = Festival.findByOffset(targetTwitterUser.twitterId, pager.pageNum * pager.maxListCount - pager.maxListCount, pager.maxListCount)
        // 全体件数
        pager.totalRows = Festival.count
    ...
    DB.withSession{ implicit session => 
      ...
      TwitterUser.findByTwitterId(twitterId.toLong) match {
        case Some(twitterUser) => {
          ...
          Festival.countByTwitterId(twitterUser.twitterId) match {
            // case countByTwitterID if countByTwitterID >= 1 => Redirect(routes.Application.festival(1, twitterUser.twitterScreenName)).flashing("error" -> "申し訳ございませんが、フェスは１件までしか登録できません！そのうち沢山登録出来るようにします！")
        ...
        case Some(twitterUser) => {
          ...
          festivalAndStageForm.bindFromRequest.fold(
            ...
            festivalAndStage => {
              ...
              Festival.insertWithStage(festival, festivalAndStage._3)
        ...
        case Some(twitterUser) => {
          ...
          Festival.findById(festivalId) match {
            ...
            case Some(festival) if festival.twitterId == twitterUser.twitterId => {
              // 削除処理実行
              Festival.delete(festival.id.get)
              Redirect(routes.Application.festival(1, twitterUser.twitterScreenName))
            }
      ...
      stageForm.bindFromRequest.fold(
        ...
        form => {
          Stage.findById(stageId) match {
            case Some(stage) => {
              val nowDate: DateTime = new DateTime
              stage.stageName = form.stageName
              stage.updateDate = Some(nowDate)
              Stage.update(stageId, stage)
            }
            case _ => Redirect(routes.Application.index(1)).flashing("error" -> " エラーが発生しました　時間をおいてから再度お試しください - ERROR CODE : updateStage 01")
          }
          Redirect(routes.Application.timetable(twitterId.toLong, festivalId))
        }
      )
    ...
    DB.withSession{ implicit session => 
      ...
      performanceForm.bindFromRequest.fold(
        ...
        performance => {
          Performance.findById(performanceId) match {
            case Some(per) => {
              per.stageId    = performance.stageId
              per.artist     = performance.artist
              per.time       = performance.time
              per.timeFrame  = performance.timeFrame
              per.updateDate = Some(new DateTime)
              Performance.update(performanceId, per)
              Redirect(routes.Application.timetable(twitterId.toLong, festivalId))
            }
            case _ => Redirect(routes.Application.index(1)).flashing("error" -> " エラーが発生しました　時間をおいてから再度お試しください - ERROR CODE : updatePerformance 01")
          }
        }
      )
    }
      ...
      Performance.findById(performanceId) match {
        case Some(performance) => {
          Festival.findById(performance.festivalId) match {
            case Some(festival) if festival.twitterId == twitterId.toLong => {
              // 削除処理実行
              Performance.delete(performance.id.get)
              Redirect(routes.Application.timetable(twitterId.toLong, festivalId))
            }
   ...
   *** TimeTable画面起動
   *****************************************************************************/
  def timetable(twitterId: Long, festivalId: Long) = DBAction { implicit request =>
  ...
  def timetableByTwitterScreenName(twitterScreenName: String, festivalId: Long) = DBAction { implicit request =>
  ...
  def deleteAll = IsAuthenticated { twitterId => implicit request =>
    DB.withSession{ implicit session => 
      ...
      TwitterUser.findByTwitterId(twitterId.toLong) match {
        case Some(twitterUser) => {
          TwitterUser.deleteAll(twitterUser.id.get)
  ...
  def usage = DBAction { implicit request =>
    // CookieからTwitterIdを取得し、取得出来た場合TwitterUserを取得する
    var twitterUser: Option[TwitterUser] = request.session.get("twitterId") match {
      case Some(twitterId) => TwitterUser.findByTwitterId(twitterId.toLong)
      case _ => None
