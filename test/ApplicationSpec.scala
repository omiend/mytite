...
import play.api.db.slick._ // DB.withSession
import scala.slick.jdbc.{GetResult, StaticQuery} // StaticQuery.updateNA(executeSql).execute

import org.joda.time.DateTime

import org.specs2.mutable._ // equalesToとかで利用
import org.specs2.runner._  // @RunWith(classOf[JUnitRunner])
import org.junit.runner._   // @RunWith(classOf[JUnitRunner])
...
// http://qiita.com/suin/items/1ea0500089ee4df9c4a3
...
class ApplicationSpec extends Specification {
  ...
  "Application" should {
    ...
    //   // --- テストデータ作成
    //   createTestDataTwitterUser
    //   createTestDataFestival
    //   createTestDataHeart
    //   createTestDataStage
    //   createTestDataPerformance
    //   // --- テスト対象実行
  ...
  def executeDdl(sqlList: String*): Unit = {
    ...
    DB.withSession { implicit session =>
      StaticQuery.updateNA(executeSql).execute
    }
  }
  /** TwitetrUser */
  val createTableTwitterUser: String =
  """
  ...
  """
  /** Festival */
  val createTableFestival: String =
  """
  ...
  /** Performance */
  val createTablePerformance: String =
  """
  ...
  def createTestDataTwitterUser = {
    DB.withSession { implicit session =>
      TwitterUser.insert(TwitterUser(
         Option(900001)
        ,900001
        ,"TEST_USER_NAME"
        ,"TEST_USER_SCREEN_NAME"
        ,"https://localhost:9000/image.jpg"
        ,"Description"
        ,"ACCESS_TOKEN"
        ,"ACCESS_TOKEN_SECRET"
        ,Some(nowDate)
  ...
  def createTestDataFestival = {
    DB.withSession { implicit session =>
      Festival.insert(Festival(
         Option(900001)
        ,900001
        ,"TEST_FESTIVAL_NAME"
        ,"TEST_FESTIVAL_DESCRIPTION"
        ,Some(nowDate)
        ,Some(nowDate)
      ))
    }
